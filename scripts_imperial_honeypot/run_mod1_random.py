...
m = ncbc.topic_model(W=x, K=30, fixed_V=True, secondary_topic=False, lambda_gem=False,
                command_level_topics=False, gamma=0.1, eta=1.0, numpyfy=True)

## Initialise the model via random initialisation
np.random.seed(123)
m.random_init()
...
## Save results
with open(results_path + 'icl_res_mod1_diri_chain_random_init.pkl', 'wb') as f:
        pickle.dump(ll, f)
        
with open(results_path + 'icl_res_mod1_diri_model_random_init.pkl', 'wb') as f:
        pickle.dump(m, f)